...
const AppShowcase = () => {
  ...
  const projectsData = [
    {
      ...
      img: "./images/project1.png", // Masla yahan tha, ab purane code ke mutabiq set kar diya hai
    },
    {
  ...
  useGSAP(() => {
    ...
    cardsRef.current.forEach((card, index) => {
      // Small check to ensure card exists before animating (good practice for dynamic arrays)
      if(card) {
        gsap.fromTo(
          card,
          {
  ...
  return (
    <div id="work" ref={sectionRef} className="app-showcase w-full py-24 relative z-10">
      ...
      {selectedProject && (
        ...
        >
          <div 
            className="relative w-full max-w-5xl mt-24 bg-[#0a0a0a] border border-purple-500/30 rounded-3xl shadow-[0_0_50px_rgba(168,85,247,0.15)] overflow-hidden flex flex-col md:flex-row transform transition-all duration-500 scale-100"
            onClick={(e) => e.stopPropagation()} // Prevent closing when clicking inside the modal
          >
            {/* Close Button */}
            <button 
              onClick={() => setSelectedProject(null)}
              className="absolute top-4 right-4 p-2 bg-black/50 hover:bg-purple-500/20 border border-transparent hover:border-purple-500/50 text-gray-400 hover:text-white rounded-full transition-all z-20"
            >
            ...
            <div className="w-full md:w-1/2 h-64 md:h-auto relative bg-zinc-900">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-[#0a0a0a] to-transparent md:bg-gradient-to-r md:from-transparent md:to-[#0a0a0a]"></div>
            </div>

            {/* Modal Content Right Side */}
            <div className="w-full md:w-1/2 p-8 md:p-12 flex flex-col justify-center relative z-10">
              <h3 className="text-3xl md:text-4xl font-extrabold mb-4 text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-fuchsia-500 leading-tight">
                {selectedProject.title}
              </h3>
              
              <div className="flex flex-wrap gap-2 mb-6">
                {selectedProject.stack.map((tech, i) => (
                  <span key={i} className="px-4 py-1.5 text-sm font-semibold text-fuchsia-300 bg-fuchsia-900/20 rounded-full border border-fuchsia-500/20 shadow-[0_0_10px_rgba(217,70,239,0.1)]">
                    {tech}
                  </span>
                ))}
              </div>
              ...
              <div className="flex flex-wrap gap-4 mt-auto">
                <button className="px-8 py-3 rounded-full bg-gradient-to-r from-purple-600 to-fuchsia-600 text-white font-bold tracking-wide hover:shadow-[0_0_20px_rgba(168,85,247,0.5)] transition-all flex items-center gap-2">
                  Live Preview
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg>
                </button>
                <button className="px-8 py-3 rounded-full border border-purple-500/50 text-purple-300 font-bold tracking-wide hover:bg-purple-900/20 transition-all flex items-center gap-2">
                  GitHub
